...
def discover_risks(simulation_id: str):
    state = engine.get_state(simulation_id)
    if state is None:
        raise HTTPException(status_code=404, detail="Simulation not found")
...
def inject_failure(simulation_id: str, scenario: str = "Payment Gateway Failure"):
    state = engine.get_state(simulation_id)
    if state is None:
        raise HTTPException(status_code=404, detail="Simulation not found")
...
def run_recovery(simulation_id: str):
    state = engine.get_state(simulation_id)
    if state is None:
        raise HTTPException(status_code=404, detail="Simulation not found")
...
def apply_chaos(simulation_id: str):
    state = engine.get_state(simulation_id)
    if state is None:
        raise HTTPException(status_code=404, detail="Simulation not found")
...
def verify_system(simulation_id: str):
    state = engine.get_state(simulation_id)
    if state is None:
        raise HTTPException(status_code=404, detail="Simulation not found")
